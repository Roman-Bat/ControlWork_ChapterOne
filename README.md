...
## Решение
Так как нам предстоит иметь дело с массивами, сначала создадим метод вывода массива, который принимает на вход строковый массив и сообщение для пользователя (название массива):
``` c#
void PrintArray(string[] array, string message)
```
Затем создадим метод, который будет проверять длину элементов массива, и если длина меньше либо равна трём символам, то записываем значение в новый массив.
``` c#
string[] NewArrayThreeSym(string[] array)
```
Вызовем методы и проверим на массивах из задания. 
